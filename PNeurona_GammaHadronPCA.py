...
#me sigue dando una probabilidad en Y_pred que no es comparable con la Y
...
net = buildNetwork(6,50,1,outclass=SigmoidLayer)
...
for i in range(len(XscalerTe)):
    r1 = net.activate([X_test0[i],X_test1[i],X_test2[i],X_test3[i],X_test4[i],X_test5[i]])
    Y_pred.append(r1)
...
print('typeYpred',type(Y_pred))

# Graficamos la curva ROC
fpr, tpr, thresholds = roc_curve(Y_test, Y_pred)
fig = plt.figure()
ax = fig.add_subplot(111)
plt.plot(fpr, tpr, lw=2, label='Roc Curve NNW', color='darkblue')
plt.axis([-0.2, 1.05, -0.05, 1.05])
for x in range(0,len(thresholds),100):
    a = round(thresholds[x],2)
    b = [fpr[x],tpr[x]]
    ax.annotate('(%s)' % a,xy=b,size='small', color='gray')
plt.title("Roc Curve NNW", color='darkred')
...
cb = fig.colorbar(res)
plt.title('Confusion Matrix NNW')
